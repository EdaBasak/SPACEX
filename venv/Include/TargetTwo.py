...
class TargetTwo():
    ...
    ExposedEvent=pygame.event.Event(pygame.USEREVENT, attr1='TargetTwoExposed')
    def __init__(self, screen):
        ...
        for i in range(1, 10):
            self.flyImages.append(
                pygame.transform.scale(pygame.image.load("images/png/Bombs/Bomb_2_Idle_"+"00" + str(i) + ".png"),
    ...
    def draw(self, screen):
        if self.explosionImageOrder == -1:
            self.flyImageOrder = (self.flyImageOrder + 1) % 9
    ...
    def hit(self,plane):
        #3 Vuruşta Öleceği için 2. Targetin canı 99. 33 azaltıyoruz
        self.life = self.life - 33
        if self.life <= 0:
            #Eğer Targetimiz ölürse Plane 5 adet cephane ekliyoruz
            plane.bulletNumber += 5
            ScoreBoard.ScoreBoard.set_Score(5)
            self.expose()
